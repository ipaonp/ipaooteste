...
$menu=Yii::app()->functions->getMerchantMenu_item($merchant_id ,  isset($_GET['item_id'])?$_GET['item_id']:'' , $todays_day ); 
echo '<input type="hidden" value="1" id="item_page"/>';
}else{
	$menu=Yii::app()->functions->getMerchantMenu($merchant_id , isset($_GET['sname'])?$_GET['sname']:'' , $todays_day );
	echo '<input type="hidden" value="0" id="item_page"/>';
}
//dump($menu);
//die();
