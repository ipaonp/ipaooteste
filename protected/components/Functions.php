...
{	
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                'url'=>array('/merchant/Settings')),
				 ...
				 array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"Settings"),'tag'=>"tablebooking",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Incoming Orders"),
                'url'=>array('/merchant/IncomingOrders')),
                ...
                /*array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"receiptSettings"),'tag'=>"receiptSettings",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Receipt Settings"),
                ...
                /*addon */
                 array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"addon"),'tag'=>'addon','label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default",'Addon'),
                   ...
                   /*array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"singleapp"),'tag'=>'singleapp', 'label'=>'<i class="fa fa-paypal"></i>'.t("Single Merchant modules"),
	...
	public function adminMenu()
	{
		return array(  
		    ...
		    'items'=>array(
                ...
                'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Cancel Orders"),
	...
	public function multiOptionToArray($json_data='')
	{
		...
		public function getItemByCategory_item($category_id='',$addon=false,$merchant_id='')
	{
		$DbExt=new DbExt;
		$data=array();
	...
	public function getMerchantMenu_item($merchant_id='',$item_id, $todays_day='')
	{		
		$data=array();
		$this->data='list';
		if ( $res=$this->getCategoryList2_item($merchant_id , $item_id , $todays_day )){						
			foreach ($res as $cat_i=>$cat_name) {				
				$data[]=array(
				  'category_id'=>$cat_i,
				  'category_name'=>$cat_name['category_name'],
				  ...
				  'category_description_trans'=>!empty($cat_name['category_description_trans'])?json_decode($cat_name['category_description_trans'],true):'',
				  'dish'=>$cat_name['dish'],
				  'item'=>$this->getItemByCategory_item($item_id,false,$merchant_id)
				);
			}
			return $data;
		}
...
'sub_item_name',$addon_name_trans).'</div>';
    		    ...
    		    if ( !empty($tax)){
    				...
    				if ($subtotal_non>=0.0001){    					
    					...
    					//$htm.="temp_subtotal=>$temp_subtotal <br/>";		    					
...
$menu_html.="</li>";
   ...
   {    	
	...
	{		
	  ...
	  public function getCategoryList2_item($merchant_id='', $food_name='', $todays_day='')
	{		
    ...
    {    	
	  ...
	  public function getdriver_review($driver_id)
    {    	
	    $stmt="SELECT a.* 
		
		FROM
			{{driver_task}} a
			WHERE a.rating!=0 and
		...
		return false;
		  ...
		  public function getdriver_review_for_merchant($order_id)
    {    	
	    $stmt="SELECT a.* 
		...
		if ( $res=$this->db_ext->rst($stmt)){
				...
				$stmt2="SELECT * FROM  {{driver}} WHERE driver_id=".$driver_id."";			      
		if ( $res2=$this->db_ext->rst($stmt2)){
			$res['driver_details']=$res2[0];
		}
			return $res;
		}
		return false;
