	    ...
	    {	  
    	    ...
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	   
    	    		...
    	    		if ( is_array($price) && count($price)>=1){
    	    			foreach ($price as $key_price=>$val_price) {    	    		
    	    				if (array_key_exists($key_price,(array)$size_list)){
    	    					$price_item=getCurrencyCode().prettyFormat($val_price);
					...
					$merchant_info=$merchant_info[0]->restaurant_slug;
					$categer_id=$val['category'];
					$categer_id=substr($categer_id, 2);
					$categer_id=substr_replace($categer_id, '',-2);
					$categer_id=str_replace('"]["', ',',$categer_id);
					$categer_id=explode('","',$categer_id);
					$categer_id=$categer_id[0];  
$social_link=$val['item_name'].'                   
'.$val['item_description'].'    
price: '.$price_item.'       
https://ipaoo.com.br/menu-'.$merchant_info.'?item_id='.$val['item_id'].'&category_id='.$categer_id;
					
					
$social_linkl='<textarea type="text" id="copyinput'.$val['item_id'].'">'.$social_link.'</textarea><button type="button" onclick="copytext('.$val['item_id'].')">Copy url</button>';
    	    		if (!empty($val['photo'])){
    	    			$img=Yii::app()->request->baseUrl."/upload/$val[photo]";
    	    		    $photo="<img class=\"uk-thumbnail uk-thumbnail-mini\" src=\"$img\" >";	
    	    		} else $photo='';
...
//dump($params); die();
	    				...
	    				if ( $this->insertData("{{order}}",$params)){
		    				...
		    				if ( $this->data['delivery_type']=="delivery"){
			    				...
			    				if(isset($this->data['map_accurate_address_lat'])){
			    					if(!empty($this->data['map_accurate_address_lat'])){
			    					   $params_address['google_lat']=$this->data['map_accurate_address_lat'];
			    					}
			    				}
			    				if(isset($this->data['map_accurate_address_lng'])){
			    					if(!empty($this->data['map_accurate_address_lng'])){
			    					   $params_address['google_lng']=$this->data['map_accurate_address_lng'];
			    					}
			    				}
			    ...
			    foreach ($res as $val) {				   	    			   	    
				   ...
				   $date=FormatDateTime($val['date_created']);
			   	   ...
			   	   $feed_data['aaData'][]=array(
			   	      ...
			   	      $val['contact_phone'],
			   	      "<span class=\"uk-text-success uk-text-small concat-text\">".$val['sms_message']."</span>",
