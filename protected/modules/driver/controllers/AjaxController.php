...
class AjaxController extends CController
{
	...
	{
		$aColumns = array(
		  'driver_id','username','first_name','email','phone','Rating',
		  'team_id','status'
		);
		$t=AjaxDataTables::AjaxData($aColumns);		
		if (isset($_GET['debug'])){
 ...
 $numberifrevuew=0;
		 ...
		 $starshow='';
		 	$ratinlist='';
		  $view_rating='';
		$review_data=Yii::app()->functions->getdriver_review($val['driver_id']);
	
	
				if($review_data){
					
					foreach($review_data as $starcount){
						if($starcount['rating']>0 and $starcount['rating']<=1){
					...
					}else if($starcount['rating']>1 and $starcount['rating']<=2){
					...
					}else if($starcount['rating']>2 and $starcount['rating']<=3){
					...
					}else if($starcount['rating']>3 and $starcount['rating']<=4){
					...
					}else if($starcount['rating']>4 and $starcount['rating']<=5){
						...
						$starshows='<span class="fa fa-star "></span><span class="fa fa-star " ></span><span class="fa fa-star " ></span><span class="fa fa-star " ></span><span class="fa fa-star" ></span>';
						 ...
						 $starcount['rating'];
						   $numberifrevuew++;
                            $totalStars += $starcount['rating'];
							$ratinlist.='<tr>';
							$ratinlist.='<td>';
								$ratinlist.=$starcount['order_id'];
							$ratinlist.='</td>';
							$ratinlist.='<td>';
							$ratinlist.=$starcount['customer_name'];
								
							$ratinlist.='</td>';
							$ratinlist.='<td>';
								 $ratinlist.=$starcount['contact_number'];
							$ratinlist.='</td>';
							$ratinlist.='<td>';
								$ratinlist.=$starcount['email_address'];
							$ratinlist.='</td>';
							$ratinlist.='<td>';
								$ratinlist.=$starcount['rating_comment'];
							$ratinlist.='</td>';
							$ratinlist.='<td>';
								$ratinlist.=$starshows;
							$ratinlist.='</td>';
					...
					if($totalStars!=0 and $numberifrevuew!=0){
					 $average =$totalStars/$numberifrevuew;
					}
					
					if($average>0 and $average<=1){
						$starshow='<span class="fa fa-star checked"></span><span class="fa fa-star "></span><span class="fa fa-star "></span><span class="fa fa-star "></span><span class="fa fa-star"></span>';
					}else if($average>1 and $average<=2){
						$starshow='<span class="fa fa-star checked"></span><span class="fa fa-star checked " ></span><span class="fa fa-star "></span><span class="fa fa-star "></span><span class="fa fa-star"></span>';
					}else if($average>2 and $average<=3){
						$starshow='<span class="fa fa-star checked"></span><span class="fa fa-star checked" ></span><span class="fa fa-star " checked></span><span class="fa fa-star "></span><span class="fa fa-star"></span>';
					}else if($average>3 and $average<=4){
						$starshow='<span class="fa fa-star checked"></span><span class="fa fa-star checked" ></span><span class="fa fa-star checked" ></span><span class="fa fa-star checked " ></span><span class="fa fa-star"></span>';
					}else if($average>4 and $average<=5){
						$starshow='<span class="fa fa-star checked"></span><span class="fa fa-star checked " ></span><span class="fa fa-star checked" ></span><span class="fa fa-star checked " ></span><span class="fa fa-star checked" ></span>';
					}else{
						$starshow='<span class="fa fa-star "></span><span class="fa fa-star " ></span><span class="fa fa-star " ></span><span class="fa fa-star " ></span><span class="fa fa-star" ></span>';
					}
					$driver_id=$val['driver_id'];
     $view_rating='<br><button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal'.$driver_id.'">'.Driver::t("View all rating").'</button>
	 <div id="myModal'.$driver_id.'" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">'.Driver::t("Driver rating").'</h4>
      </div>
      <div class="modal-body">
       <table class="table table-hover dataTable">
	        <thead>
	        <tr>
			    <th>'.Driver::t("order id").'</th>
				
				<th>'.Driver::t("client Name").'</th>
				<th>'.Driver::t("Phone").'</th>
				<th>'.Driver::t("client email").'</th>
				<th>'.Driver::t("client Feedback").'</th>
				<th style="    width: 144px;">'.Driver::t("rating").'</th>
			<tr>
			</thead>
			<tbody>
			'.$ratinlist.'
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">'.Driver::t("Close").'</button>
      </div>
    </div>
	 ...
	 ';
			    ...
			    $feed_data['aaData'][]=array(
			      ...
			      $val['email'],
			      $val['phone'],
				  $starshow.'<br>Rating:'.round($average,1).' '.$view_rating
				  ,
			      $val['team_name'],
	...
	public function actiontaskList()
	{
		$aColumns = array(
		  ...
		  'driver_name','Driver_rating','customer_name','delivery_address','delivery_date'
		);
		$t=AjaxDataTables::AjaxData($aColumns);		
		if (isset($_GET['debug'])){
		    dump($t);
		...
		if ( $res=$DbExt->rst($stmt)){
		 ...
		 $starshow='';
		 	$ratinlist='';
		  $view_rating='';
		$review_data=Yii::app()->functions->getdriver_review_for_merchant($val['order_id']);
	...
	//print_r($review_data);
				if($review_data){
				  ...
				  $view_rating='<br><button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal'.$val['order_id'].'">'.Driver::t("View rating").'</button>
	 <div id="myModal'.$val['order_id'].'" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       <table class="table table-hover dataTable">
	        ...
	        <tr>
				...
				<th>'.Driver::t("Driver ID").'</th>
				
					<th>'.Driver::t("Driver name").'</th>
					<th>'.Driver::t("Phone").'</th>
					<th>'.Driver::t("client Feedback").'</th>
					<th>'.Driver::t("rating").'</th>
	 ...
	 ';
			if($ratinlist=='0'){
				$view_rating=Driver::t("No rating Found");
			}	
