...
{	
    ...
    {        
		...
		if ($Validator->validate()){
		 ...
		 $starshow='';
		 	$ratinlist='';
		  $view_rating='';
		$review_data=Yii::app()->functions->getdriver_review_for_merchant($this->data['order_id']);
				...
				if($review_data){
			    	...
			    	array(
					  ...
					  if ( Yii::app()->functions->code==1){					  	  
						    ...
						    $data_raw['driver_rating']=$ratinlist;
	...
	public function actionsendnoty()
	{
		...
		$req=array(
		  'token'=>$this->t("token is required"),
		  'mtid'=>$this->t("merchant id is required"),
		  'push_message'=>$this->t("push message is required")
		);
		...
		if ($Validator->validate()){
         ...
         $merchant_info=Yii::app()->functions->getMerchant($this->data['mtid']);

				
			    $params = array(
    	   'push_title'=>$merchant_info['restaurant_name'],
    	   'push_message'=>$this->data['push_message'],
    	   'device_platform'=>'3',
		   'merchant_id'=>$this->data['mtid'],
		   'client_select'=>$this->data['select_customers'],
    	   'date_created'=>FunctionsV3::dateNow(),
    	   'ip_address'=>$_SERVER['REMOTE_ADDR']
    	);
    	$db=new DbExt();
