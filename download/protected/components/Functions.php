	...
	public function merchantMenu()
	{
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"category_sked"),'tag'=>"category_sked",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Cronograma por categoria"),
...
$menu_html.="</li>";
   ...
   {    	
	...
	public function AAccess()
	{
		...
		if (is_object($info)){
			$access = false;
			if (!empty($info->user_access)) {
				if (is_array($info->user_access) || is_object($info->user_access)) {
					$access = json_decode($info->user_access);
				} else {
					$remover = array("\"", "[", "]");
					$access = explode(",", str_replace($remover, "", $info->user_access));
				}
			}
